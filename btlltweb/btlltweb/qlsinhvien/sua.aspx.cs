...
namespace btlltweb.qlsinhvien
{
    public partial class sua : System.Web.UI.Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            conn.ConnectionString = @"Provider=Microsoft.Jet.OLEDB.4.0; Data Source=D:\btlltweb\qlysinhvien.mdb";
            conn.Open();
            string sql = string.Format(@"UPDATE SINH_VIEN SET HoVaTen = '{0}' , NgaySinh = '{1}' , GioiTinh = '{2}' , QueQuan = '{3}' , SDT = '{4}' , Email = '{5}'  , MaLop = '{6}' where MaSinhVien ='{7}'", hvt.Text, ns.Text, gt.Text, qq.Text, sdt.Text, email.Text, ml.Text, Msv.SelectedValue);
            OleDbCommand cmd = new OleDbCommand(sql, conn);
            cmd.ExecuteNonQuery();
            string script = "alert('Sinh viên đã được sửa');";
