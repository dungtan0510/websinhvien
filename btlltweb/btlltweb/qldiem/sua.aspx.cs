...
namespace btlltweb.qldiem
{
    public partial class sua : System.Web.UI.Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            conn.ConnectionString = @"Provider=Microsoft.Jet.OLEDB.4.0; Data Source=D:\btlltweb\qlysinhvien.mdb";
            conn.Open();
            string sql = string.Format(@"UPDATE DIEM SET LanHoc = '{0}' , LanThi = '{1}' , Diem = '{2}'  where MaSinhVien ='{3}' AND MaMonHoc = '{4}'", lh.Text, lt.Text, diem.Text, msv.SelectedValue, Mmh.SelectedValue);
            OleDbCommand cmd = new OleDbCommand(sql, conn);
            cmd.ExecuteNonQuery();
            string script = "alert('Đã sửa điểm thành công');";
