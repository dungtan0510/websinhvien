...
namespace btlltweb.qlmonhoc
{
    public partial class sua : System.Web.UI.Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            conn.ConnectionString = @"Provider=Microsoft.Jet.OLEDB.4.0; Data Source=D:\btlltweb\qlysinhvien.mdb";
            conn.Open();
            string sql = string.Format(@"UPDATE MON_HOC SET TenMonHoc = '{0}' , SoTinChi = {1}  where MaMonHoc ='{2}'", tmh.Text, stc.Text, Mmh.SelectedValue);
            OleDbCommand cmd = new OleDbCommand(sql, conn);
            cmd.ExecuteNonQuery();
            string script = "alert('Môn học đã được sửa');";
