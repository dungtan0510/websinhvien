...
namespace btlltweb.qllophoc
{
    public partial class sua : System.Web.UI.Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            conn.ConnectionString = @"Provider=Microsoft.Jet.OLEDB.4.0; Data Source=D:\btlltweb\qlysinhvien.mdb";
            conn.Open();
            string sql = string.Format(@"UPDATE LOP_HOC SET TenLop = '{0}' , Khoa = '{1}' , NamVao = {2}  where MaLop ='{3}'", tl.Text, khoa.Text, namvao.Text, ml.SelectedValue);
            OleDbCommand cmd = new OleDbCommand(sql, conn);
            cmd.ExecuteNonQuery();
            string script = "alert('Lớp Học đã được sửa');";
